...
// Resposta está na função espelho()
package Espelho;
...
class No<T>{
	...
	public class BSTespelho<T extends Comparable<T>>{
		private No<T> raiz;
		...
		public void espelho(T valor) {
			...
			if(raiz == null) {
				...
				No<T> atual = this.raiz;
				while(true) {
					if(novoElem.getValor().compareTo(atual.getValor()) == 1) { //maior
						if(atual.getEsq() != null) { //vai para esquerda
							atual = atual.getEsq();
						}else {
							atual.setEsq(novoElem);
	...
	public static void main(String[] args) {
		BSTespelho<Integer> arvore = new BSTespelho<Integer>();
		BSTespelho<Integer> t = new BSTespelho<Integer>();
		...
		if( elementos > 0 && elementos < 1000) {
			...
			System.out.println("Árvore espelho pré ordenada");
